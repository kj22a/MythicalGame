public class Bonus extends Population {
    ...
    Bonus(int mageVal, int archerVal, int halberdierVal, int cavalryVal, char civilisation) {
    ...
    int[] calculator() {
        ...
        if (civilisation == 'A' || civilisation == 'a') {
            cavTuple[0] *= 2;
            cavTuple[1] *= 2;
            return new int[] {cavTuple[0] + arcTuple[0] + halTuple[0] + magTuple[0], cavTuple[1] + arcTuple[1] + halTuple[1] + magTuple[1]};
        } else if(civilisation == 'B' || civilisation == 'b') {
            magTuple[0] *= 2;
            magTuple[1] *= 2;
            return new int[] {cavTuple[0] + arcTuple[0] + halTuple[0] + magTuple[0], cavTuple[1] + arcTuple[1] + halTuple[1] + magTuple[1]};
        }
        else if(civilisation == 'M' || civilisation == 'm') {
            arcTuple[0] *= 2;
            arcTuple[1] *= 2;
            return new int[] {cavTuple[0] + arcTuple[0] + halTuple[0] + magTuple[0], cavTuple[1] + arcTuple[1] + halTuple[1] + magTuple[1]};
        } else {
